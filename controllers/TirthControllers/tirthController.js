...
const reviewApplication = async (req, res) => {
    try {
        ...
        if (status === 'approved' && tirth.manager && tirth.manager.jainAadharNumber) {
            const User = require('../../models/UserRegistrationModels/userModel');
            const user = await User.findOne({ jainAadharNumber: tirth.manager.jainAadharNumber });
...
const getAllTirths = async (req, res) => {
    try {
        const tirths = await Tirth.find(
            ...
            'name tirthType location description uploadImage'
        ).sort({ name: 1 });
