...
// Validate Sangh ID and get Sangh details
// const validateSangh = asyncHandler(async (req, res) => {
//     try {
...
//         return successResponse(res, sangh, 'Sangh found successfully');
//     } catch (error) {
//         return errorResponse(res, error.message, 500);
//     }
// });

// // Add new Panch member
// const addPanchMember = asyncHandler(async (req, res) => {
//     try {
//         const { sanghId } = req.params;
//         const { personalDetails } = req.body;

//         // 1. Verify Sangh exists
//         const sangh = await Sangh.findById(sanghId);
//         if (!sangh) {
//             return errorResponse(res, 'Sangh not found', 404);
//         }

//         // 2. Check current Panch member count
//         const currentPanchCount = await Panch.countDocuments({ 
//             sanghId, 
//             status: 'active'
//         });

//         if (currentPanchCount >= 5) {
//             return errorResponse(res, 'Maximum limit of 5 Panch members has been reached', 400);
//         }

//         // 3. Check if the person exists in Sangh members
//         const isSanghMember = sangh.members.some(member => 
//             member.jainAadharNumber === personalDetails.jainAadharNumber
//         );

//         if (!isSanghMember) {
//             return errorResponse(res, 'This person is not a member of this Sangh. Only Sangh members can be appointed as Panch.', 400);
//         }

//         // 4. Check if already a Panch member
//         const existingPanchMember = await Panch.findOne({
//             sanghId,
//             'personalDetails.jainAadharNumber': personalDetails.jainAadharNumber
//         });

//         if (existingPanchMember) {
//             return errorResponse(res, 'This person is already a Panch member', 400);
//         }

//         // 5. Create new Panch member
//         const panchMember = await Panch.create({
//             sanghId,
//             personalDetails,
//             documents: {
//                 jainAadharPhoto: req.files?.jainAadharPhoto?.[0]?.location,
//                 profilePhoto: req.files?.profilePhoto?.[0]?.location
//             }
//         });

//         return successResponse(res, panchMember, 'Panch member added successfully', 201);
//     } catch (error) {
//         // Delete uploaded files if member creation fails
//         if (req.files) {
//             const deletePromises = Object.values(req.files).map(async (fileArray) => {
